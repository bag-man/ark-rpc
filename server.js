...
function restrictToLocalhost (req, res, next) {
  if (req.connection.remoteAddress === '::1' || req.connection.remoteAddress === '127.0.0.1' || req.connection.remoteAddress === '::ffff:127.0.0.1') { next() } else res.end()
}
...
if (program.port) { startServer(program.port) } else { startServer(1337) }
